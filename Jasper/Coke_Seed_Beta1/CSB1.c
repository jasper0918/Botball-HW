...
const int ONE_CIRCLE = 950;		// ticks needed to accomplish a circle
...
// servo constants:
const int BSV_LEVEL = 1750;			// box servo keeps level
...
const int OFFSET_Y = 70;	// standard y coordinate to catch a ball
const int P_X = 1;
const int P_Y = 1;
	...
	while(!(a_button_clicked())){
		...
		if (objNum >= 2 && get_object_area(GREEN, 1) > 200)
		{
			...
			if (objCen_Ball1.y >= (objCen_Ball2.y + 2))
				objCen = objCen_Ball1;
			else if (objCen_Ball1.x <= objCen_Ball2.x)
				objCen = objCen_Ball1;
			else
				objCen = objCen_Ball2;
		}
