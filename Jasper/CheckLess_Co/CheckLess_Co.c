...
const int RM_MINS = 37;			// minimum motor speed
...
const int ASV_CHECK = 1050;		// arm servo rises for check
...
const int SSV_BACK = 122;			// small servo keeps back
	...
	*/
	while(1){
		...
		if (objNum >= 2 && get_object_area(GREEN, 1) > 200)
		{
			...
			if (objCen_Ball1.x <= objCen_Ball2.x + 2)
				objCen = objCen_Ball1;
			else if (objCen_Ball1.y >= objCen_Ball2.y + 2)
				objCen = objCen_Ball1;
			else
				objCen = objCen_Ball2;
		}
...
int check(){
	camera_update();
	camera_update();
	if(get_object_area(GREEN, 0) > CHECK_AREA)
		return GREEN;
	if(get_object_area(PINK, 0) > CHECK_AREA)
		return PINK;
	else
		return 3;
}
