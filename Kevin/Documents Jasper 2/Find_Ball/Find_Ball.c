...
void findBall(){
	display_clear();
	int temptSpeed = MINS;			// changeable minimum speed
	...
	while(!(a_button_clicked())){
		...
		// make sure there is really a ball in sight
		...
		// check if speed is too high or low and make modification to minimum speed
		
		if(turnLM > -1 * temptSpeed && turnLM < 0)
			turnLM = -1 * temptSpeed;
		if(turnLM > 0 && turnLM < temptSpeed)
			turnLM = temptSpeed;
		if(turnRM > -1 * temptSpeed && turnRM < 0)
			turnRM = -1 * temptSpeed;
		if(turnRM > 0 && turnRM < temptSpeed)
			turnRM = temptSpeed;
		// avoid unmoving
...
int check(int color){
	...
	// the ball is wrong and pushed out
